...
While it was primarily created for Revcord, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @revcord/discord-types
yarn add -D @revcord/discord-types
pnpm add -D @revcord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@revcord/discord-types";
...
```ts
import { ApplicationCommandType } from "@revcord/discord-types/enums";
