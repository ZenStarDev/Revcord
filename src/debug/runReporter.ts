    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                ...
                replace: "(Revcord.Webpack._initReporter(),$&)"
            }
        }, "Revcord Reporter");

        // @ts-expect-error
        Revcord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$revcordGetWrappedComponent != null && result.$$revcordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$revcordProps != null) {
                        logMessage += `(${args[0].$$revcordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the Revcord object has been created.
// We need to add extra properties to it, and it is only created after all of Revcord code has ran
