...
export async function checkForUpdates() {
    changes = await Unwrap(RevcordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(RevcordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(RevcordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(RevcordNative.updater.getRepo());
