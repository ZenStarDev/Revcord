...
export async function checkCloudUrlCsp() {
    ...
    if (host === "api.revcord.dev") return true;

    if (await RevcordNative.csp.isDomainAllowed(Settings.cloud.url, ["connect-src"])) {
        return true;
    }

    const res = await RevcordNative.csp.requestAddOverride(Settings.cloud.url, ["connect-src"], "Cloud Sync");
    if (res === "ok") {
        openModal(props => (
            <ConfirmModal
                {...props}
...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("Revcord_cloudSecret") ?? {};

    const origin = getCloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("Revcord_cloudSecret", secrets => {
            secrets ??= {};
