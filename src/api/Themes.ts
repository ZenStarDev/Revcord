...
import { ThemeStore } from "@revcord/discord-types";
import { createAndAppendStyle } from "@utils/css";
import { PopoutWindowStore } from "@webpack/common";

import { revcordRootNode,userStyleRootNode } from "./Styles";
...
async function toggle(isEnabled: boolean) {
    if (!style) {
        if (isEnabled) {
            style = createAndAppendStyle("revcord-custom-css", userStyleRootNode);
            RevcordNative.quickCss.addChangeListener(css => {
                style.textContent = css;
                // At the time of writing this, changing textContent resets the disabled state
                style.disabled = !Settings.useQuickCss;
                updatePopoutWindows();
            });
            style.textContent = await RevcordNative.quickCss.get();
        }
    } else
...
async function initThemes() {
    themesStyle ??= createAndAppendStyle("revcord-themes", userStyleRootNode);
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            const themeData = await RevcordNative.themes.getThemeData(theme);
            if (!themeData) continue;
            const blob = new Blob([themeData], { type: "text/css" });
            links.push(URL.createObjectURL(blob));
        }
    } else {
        const localThemes = enabledThemes.map(theme => `revcord:///themes/${theme}?v=${Date.now()}`);
...
function applyToPopout(popoutWindow: Window | undefined, key: string) {
    ...
    doc.querySelector("revcord-root")?.remove();

    doc.documentElement.appendChild(revcordRootNode.cloneNode(true));
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        RevcordNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });
