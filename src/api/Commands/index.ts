...
import { CommandArgument, CommandContext, CommandOption } from "@revcord/discord-types";
import { Logger } from "@utils/Logger";
import { makeCodeblock } from "@utils/text";

import { sendBotMessage } from "./commandHelpers";
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, RevcordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: RevcordCommand[];
export const commands = {} as Record<string, RevcordCommand>;
...
export const _init = function (cmds: RevcordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: RevcordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isRevcordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | RevcordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: RevcordCommand, plugin: string) {
...
export function registerCommand<C extends RevcordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
