...
import { Channel, RenderModalProps } from "@revcord/discord-types";
...
const AdditionalAllowedChannelIds = [
    "1024286218801926184", // Revcord > #bot-commands
];
...
async function generateDebugInfoMessage() {
    ...
    const info = {
        Revcord:
            `v${VERSION} • [${gitHash}](<https://github.com/Vendicated/Revcord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Revcord DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Forms.FormText>You are using a custom build of Revcord, which we do not provide support for!</Forms.FormText>

                <Forms.FormText className={Margins.top8}>
                    We only provide support for <Link href="https://revcord.dev/download">official builds</Link>.
                    Either <Link href="https://revcord.dev/download">switch to an official build</Link> or figure your issue out yourself.
                </Forms.FormText>
...
export default definePlugin({
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Forms.FormText>You are using an outdated version of Revcord! Chances are, your issue is already fixed.</Forms.FormText>
                                <Forms.FormText className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Forms.FormText>You are using an externally updated Revcord version, which we do not provide support for!</Forms.FormText>
                            <Forms.FormText className={Margins.top8}>
                                Please either switch to an <Link href="https://revcord.dev/download">officially supported version of Revcord</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        if (props.channel.parent_id === SUPPORT_CATEGORY_ID && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/revcord-debug") || props.message.content.includes("/revcord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Revcord plugin developers for support!
                <br />
                Instead, use the Revcord support channel: {Parser.parse("https://discord.com/channels/1015060230222131221/1026515880080842772")}
