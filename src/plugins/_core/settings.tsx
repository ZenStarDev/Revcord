...
import { BackupAndRestoreTab, CloudTab, PatchHelperTab, PluginsTab, RevcordTab,ThemesTab, UpdaterTab } from "@components/settings/tabs";
...
const settings = definePluginSettings({
    settingsLocation: {
        ...
        description: "Where to put the Revcord settings section",
        options: [
    ...
    includeRevcordInfoWhenCopying: {
        type: OptionType.BOOLEAN,
        description: "Also copy Revcord info (Revcord, Electron, Chromium) when clicking the version info in the bottom left area of the Settings page",
        default: true
    }
});
...
export default definePlugin({
    ...
    buildLayout(originalLayoutBuilder: SettingsLayoutBuilder) {
        ...
        if (layout.some(s => s?.key === "revcord_section")) return layout;

        const { buildEntry } = this;

        const revcordEntries: SettingsLayoutNode[] = [
            buildEntry({
                key: "revcord_main",
                title: "Revcord",
                panelTitle: "Revcord Settings",
                Component: RevcordTab,
            ...
            buildEntry({
                key: "revcord_cloud",
                title: "Cloud",
                panelTitle: "Revcord Cloud",
                Component: CloudTab,
                Icon: CloudIcon
            }),
            buildEntry({
                key: "revcord_backup_restore",
                title: "Backup & Restore",
                Component: BackupAndRestoreTab,
                Icon: BackupRestoreIcon
            }),
            !IS_STANDALONE && PatchHelperTab && buildEntry({
                key: "revcord_patch_helper",
                title: "Patch Helper",
            ...
            ...this.customSections.map((func, i) => {
                ...
                return buildEntry({
                    key: `revcord_deprecated_custom_${section}`,
                    title: label,
                    Component: element,
        ...
        const revcordSection: SettingsLayoutNode = {
            key: "revcord_section",
            type: LayoutTypes.SECTION,
            useTitle: () => "Revcord Settings",
            buildLayout: () => revcordEntries
        };
        ...
        layout.splice(idx, 0, revcordSection);
    ...
    get electronVersion() {
        return RevcordNative.native.getVersions().electron || window.legcord?.electron || null;
    },

    get chromiumVersion() {
        try {
            return RevcordNative.native.getVersions().chrome
    ...
    getInfoRows() {
        ...
        const rows = [`Revcord ${gitHash}${additionalInfo}`];
    ...
    getInfoString() {
        if (!settings.store.includeRevcordInfoWhenCopying) return "";
