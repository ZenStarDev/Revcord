...
// Only import pure modules that don't import other parts of Revcord.
...
const setCssDebounced = debounce((css: string) => RevcordNative.quickCss.set(css));

const themeStore = DataStore.createStore("RevcordThemes", "RevcordThemeData");

// probably should make this less cursed at some point
window.RevcordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Revcord" }),
    ...
    quickCss: {
        get: () => DataStore.get("RevcordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("RevcordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "RevcordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => RevcordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("RevcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("RevcordSettings", JSON.stringify(s)),
